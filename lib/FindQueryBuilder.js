...
class FindQueryBuilder {
  ...
  _buildJoins(params, builder) {
    _.each(params, (param) => {
      _.each(param.propertyRefs, (ref) => {
        _.each(ref.relations, (rel) => {
          ...
          if (param.filter){
            filter = ref._getFilter(param, this._modelClass);
          }

          const standardizedTableName = standardizingTableName(
            rel.relatedModelClass.getTableName()
          );

          if (!this._aliases[standardizedTableName]) {
            this._aliases[standardizedTableName] = standardizedTableName + '_alias';

            if (filter?.args.length && 
                filter.args[0].split('.')[0] === this._aliases[standardizedTableName] && 
                filter.args[0].includes('deleted_at')){

                if (!builder.context().hasDeleted){
                  builder.context().hasDeleted = [];
                }
                builder.context().hasDeleted.push(rel.relatedModelClass.getTableName());
            }

            rel.join(builder, {
              joinOperation: 'innerJoin',
              relatedTableAlias: this._aliases[standardizedTableName],
              ownerTable:
                this._aliases[standardizingTableName(rel.ownerModelClass.getTableName())] ||
                rel.ownerModelClass.getTableName(),
            });
          }
        });
      });
    ...
    if (this._aliases.length) {
      builder.select(this._modelClass.getTableName() + '.*');
    }
  }
  ...
  _buildGroupBy(params, builder) {
    ...
    if (groupByParam) {
      const groupByValues = groupByParam.value.split(',');
      const aliasedGroupBy = [];

      _.each(groupByValues, (value) => {
        const propertyRef = this._parsePropertyRef(value);
        const modelClass = propertyRef.modelClasses.at(-1) || propertyRef._modelClass;
        const tablename = modelClass?.getTableName();
        const tableAlias = this._aliases[standardizingTableName(tablename)] || tablename;
        const fieldAlias =
          modelClass.columnNameAlias?.[_.camelCase(propertyRef.columnName)] ||
          propertyRef.columnName;

        const alias = (tableAlias ? tableAlias + '.' : '') + fieldAlias;
        aliasedGroupBy.push(alias);
      });

      builder.groupBy(aliasedGroupBy);
    }
  }
  ...
  _buildOrderBy(params, builder) {
    _.each(params, (param) => {
      ...
      if (orderType && orderType.indexOf('orderBy') !== -1) {
        ...
        const rel = propertyRef.relations.at(-1);
        ...
        if (rel) {
          ...
          builder.orderBy(
            this._aliases[standardizingTableName(rel.relatedModelClass.getTableName())] +
            '.' +
...
function standardizingTableName(tablename) {
